...
class Ray:
    ...
    IntersectionResult = namedtuple("IntersectionResult",
                                    "success distance intersection normal item")
    ...
    def intersect(self, obj) -> IntersectionResult:
        if isinstance(obj, Sphere):
            ...
            return Ray.IntersectionResult(success=False,
                                          distance=0,
                                          intersection=None,
                                          normal=None,
                                          item=None)
    ...
    def _intersect_sphere(self, sphere : Sphere) -> IntersectionResult:
        ...
        no_intersect = Ray.IntersectionResult(success=False,
                                              distance=0,
                                              intersection=None,
                                              normal=None,
                                              item=None)
        if discriminant < 0.0:
            ...
            return Ray.IntersectionResult(success=True,
                                          distance=distance,
...
class PathTracer:
    def start(self, camera : Camera,
        ...
        with Pool(processes=4) as pool:
            colors = pool.imap(path_trace_from_tuple, args_generator(), samples * 75)
            for j in range(camera.resolution[1]):
                for i in range(camera.resolution[0]):
                    for s in range(samples):
                        img[j][i] += next(colors)
